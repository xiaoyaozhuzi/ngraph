...
{
public:
    ExecNode(std::shared_ptr<Node> node)
        : m_node{node}
    {
    }
    virtual ~ExecNode() {}
    static std::shared_ptr<ExecNode> create_exec(const std::shared_ptr<Node>& node);

    virtual OP_TYPEID get_typeid() const = 0;

    void execute(const element::Type& type,

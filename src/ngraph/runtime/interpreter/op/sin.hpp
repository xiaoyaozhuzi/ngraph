...
public:
    static std::shared_ptr<ExecNode> create(const std::shared_ptr<ngraph::Node>& node)
    {
        ...
        return std::static_pointer_cast<ExecNode>(std::make_shared<SinExec>(node));
    }

    SinExec(const std::shared_ptr<ngraph::Node> node)
        : ExecNode{node}
        , m_node{std::dynamic_pointer_cast<const ngraph::op::Sin>(node)}
    {
    ...
    OP_TYPEID get_typeid() const override { return OP_TYPEID::Sin_TYPEID; }
private:
    std::shared_ptr<const ngraph::op::Sin> m_node;
};

...
public:
    static std::shared_ptr<ExecNode> create(const std::shared_ptr<ngraph::Node>& node)
    {
        ...
        return std::static_pointer_cast<ExecNode>(std::make_shared<SoftmaxExec>(node));
    }

    SoftmaxExec(const std::shared_ptr<ngraph::Node> node)
        : ExecNode{node}
        , m_node{std::dynamic_pointer_cast<const ngraph::op::Softmax>(node)}
    {
    ...
    OP_TYPEID get_typeid() const override { return OP_TYPEID::Softmax_TYPEID; }
private:
    std::shared_ptr<const ngraph::op::Softmax> m_node;
};

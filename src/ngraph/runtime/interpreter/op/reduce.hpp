...
public:
    static std::shared_ptr<ExecNode> create(const std::shared_ptr<ngraph::Node>& node)
    {
        ...
        return std::static_pointer_cast<ExecNode>(std::make_shared<ReduceExec>(node));
    }

    ReduceExec(const std::shared_ptr<ngraph::Node> node)
        : ExecNode{node}
        , m_node{std::dynamic_pointer_cast<const ngraph::op::Reduce>(node)}
    {
    ...
    OP_TYPEID get_typeid() const override { return OP_TYPEID::Reduce_TYPEID; }
private:
    std::shared_ptr<const ngraph::op::Reduce> m_node;
};

...
public:
    ...
    {
        std::cout << "create MaxPool" << std::endl;
        return std::static_pointer_cast<ExecNode>(std::make_shared<MaxPoolExec>(node));
    }

    MaxPoolExec(const std::shared_ptr<ngraph::Node> node)
        : ExecNode{node}
        , m_node{std::dynamic_pointer_cast<const ngraph::op::MaxPool>(node)}
    {
        (void)m_node; // Silence compiler warning

        std::cout << "MaxPool ctor" << std::endl;
    }
    ...
    {
        std::cout << "execute MaxPool" << std::endl;
    }

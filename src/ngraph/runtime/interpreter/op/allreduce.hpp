...
class ngraph::runtime::interpreter::AllReduceExec : public ExecNode
{
public:
    static std::shared_ptr<ExecNode> create(const std::shared_ptr<ngraph::Node>& node)
    {
        return std::static_pointer_cast<ExecNode>(std::make_shared<AllReduceExec>(node));
    }

    AllReduceExec(const std::shared_ptr<ngraph::Node> node)
        : ExecNode{node}
        , m_node{std::dynamic_pointer_cast<const ngraph::op::AllReduce>(node)}
    {
        (void)m_node; // Silence compiler warning
    }

    virtual ~AllReduceExec() {}
    template <typename T>

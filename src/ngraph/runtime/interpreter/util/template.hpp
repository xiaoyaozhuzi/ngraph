...
public:
    static std::shared_ptr<ExecNode> create(const std::shared_ptr<ngraph::Node>& node)
    {
        ...
        return std::static_pointer_cast<ExecNode>(std::make_shared<CAMEL_CASE_NAMEExec>(node));
    }

    CAMEL_CASE_NAMEExec(const std::shared_ptr<ngraph::Node> node)
        : ExecNode{node}
        , m_node{std::dynamic_pointer_cast<const ngraph::op::CAMEL_CASE_NAME>(node)}
    {
    ...
    OP_TYPEID get_typeid() const override { return OP_TYPEID::CAMEL_CASE_NAME_TYPEID; }
private:
    std::shared_ptr<const ngraph::op::CAMEL_CASE_NAME> m_node;
};
